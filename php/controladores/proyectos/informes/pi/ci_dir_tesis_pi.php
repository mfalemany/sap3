<?php
class ci_dir_tesis_pi extends sap_ci
{
	...
	function evt__guardar()
	{
		try {
			...
			$this->get_datos('pi_inf_dir_tesis')->set(array('id_informe'=>$informe['id_informe']));
	...
	{
		$trabs = toba::consulta_php('co_proyectos_informes')->get_direcciones_tesis('pi',array('id_informe'=>$this->informe_actual['id_informe']));
	...
	function evt__cu_tesistas__borrar($seleccion)
	{
		...
		try {
			$this->get_datos()->cargar($seleccion);
			$this->get_datos('pi_inf_dir_tesis')->cargar(array('id_informe'=>$this->informe_actual['id_informe']));
