<?php
class ci_trab_difusion_pdts extends sap_ci
{
	...
	function evt__guardar()
	{
		try {
			...
			$this->get_datos('pdts_inf_trab_dif')->set(array('id_informe'=>$informe['id_informe']));
	...
	function conf__cu_trab_difusion(sap_ei_cuadro $cuadro)
	{
		$trabs = toba::consulta_php('co_proyectos_informes')->get_trabajos_difundidos('pdts',array('id_informe'=>$this->informe_actual['id_informe']));
		$cuadro->set_datos($trabs);
		
	}

	function evt__cu_trab_difusion__seleccion($seleccion)
	{
		$this->get_datos()->cargar($seleccion);
		$this->set_pantalla('pant_edicion');
	}

	function evt__cu_trab_difusion__borrar($seleccion)
	{
		$this->informe_actual = $this->controlador()->get_datos('proyectos_pdts_informe')->get();
		try {
			$this->get_datos()->cargar($seleccion);
			$this->get_datos('pdts_inf_trab_dif')->cargar(array('id_informe'=>$this->informe_actual['id_informe']));
	...
	function conf__cu_trab_difusion_anteriores(sap_ei_cuadro $cuadro)
	{
		$cuadro->colapsar();
		$trabs = toba::consulta_php('co_proyectos_informes')->get_trabajos_difundidos('pdts',array('previos_a_id_informe'=>$this->informe_actual['id_informe']));
		$cuadro->set_datos($trabs);
	...
	function conf__form_trab_dif(sap_ei_formulario $form)
	{
		$datos = $this->get_datos('trab_difundido')->get();
		if($datos){
			$form->set_datos($datos);
		}
	}

	function evt__form_trab_dif__modificacion($datos)
	{
		$this->get_datos('trab_difundido')->set($datos);
	}
	...
	function conf__ml_trab_dif_autores(sap_ei_formulario_ml $form_ml)
	{
		$datos = $this->get_datos('trab_difundido_autor')->get_filas();
		if(count($datos)){
			$form_ml->set_datos($datos);
		}
	}

	function evt__ml_trab_dif_autores__modificacion($datos)
	{
		$this->get_datos('trab_difundido_autor')->procesar_filas($datos);
	}
	...
	{
		return ($tabla) ? $this->dep('datos_difun')->tabla($tabla) : $this->dep('datos_difun');
	}
	...
	function validar_autores()
	{
		...
		$autores = array_column($this->get_datos('trab_difundido_autor')->get_filas(),'nro_documento');
