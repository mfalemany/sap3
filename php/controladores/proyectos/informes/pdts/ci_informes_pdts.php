<?php
class ci_informes_pdts extends sap_ci
{
	...
	function conf()
	{
		...
		$this->informe_actual = $this->get_datos('proyectos_pdts_informe')->get();
	...
	function conf__pant_general(toba_ei_pantalla $pantalla)
	{
		...
		$template = $this->armar_template(__DIR__ . "/../templates/template_informacion_general_pdts.php",$datos_template);
	...
	{
		$datos = $this->get_datos('proyectos_pdts_informe')->get();
		...
		$fecha_presentacion = (isset($datos['fecha_presentacion']) && $datos['fecha_presentacion'])
								...
								: date('Y-m-d');
		$this->get_datos('proyectos_pdts_informe')->set(
	...
	{
		$this->get_datos('proyectos_pdts_informe')->set($datos);
	}
	...
	function conf__form_pdts(sap_ei_formulario $form)
	{
		$form->set_datos($this->informe_actual);
	...
	function evt__form_pdts__modificacion($datos)
	{
		$this->get_datos('proyectos_pdts_informe')->set($datos);
	}
	...
	function conf__form_pdts_gestion(sap_ei_formulario $form)
	{
		$form->set_template(file_get_contents(__DIR__ . '/../templates/template_form_pdts_gestion.php'));
		$form->set_datos($this->informe_actual);
	...
	function evt__form_pdts_gestion__modificacion($datos)
	{
		$this->get_datos('proyectos_pdts_informe')->set($datos);
	}
	...
	function conf__ml_integ_pdts(sap_ei_formulario_ml $form_ml)
	{
		if( ! $this->soy_admin()){
		...
		if( ! $datos){
			...
			if(isset($this->informe_actual['id_informe'])){
				$evs = toba::consulta_php('co_proyectos_informes')->get_evaluaciones($this->informe_actual['id_informe'],'pdts');
	...
	function evt__ml_integ_pdts__modificacion($datos)
	{
	...
	{
		return $this->controlador()->get_datos_pdts($tabla);
	}
	...
	function generar_evaluaciones_integrantes()
	{
		foreach($this->proyecto['integrantes'] as $integrante){
			...
			$this->get_datos('sap_pdts_inf_integ_eval')->nueva_fila();
		}
	}
	...
	{
		$obligatorios = array(
			array('pantalla'=>'Gestión del Proyecto','dt'=>'proyectos_pdts_informe','campo'=>'apoyo_monto_recibido','campo_desc'=>'Apoyo económico recibido')
		);
