	...
	<fieldset>
		...
		<div id="info_admisibilidad">
			<div class="caja_info_warning">
				...
				<p>Los postulantes "No Admitidos" que consideren necesario presentar recursos de reconsideración, tendr&aacute;n plazo hasta el <?php echo strftime('%d de %B de %Y',strtotime($datos['fecha_limite_reconsideracion'])); ?> - 13:00hs, mediante una nota dirigida a <?php echo ($datos['genero_secretario'] == 'M') ? 'el' : 'la'; ?> <?php echo ($datos['genero_secretario'] == 'M') ? 'Secretario' : 'Secretaria'; ?> General de Ciencia y Técnica de la UNNE, <?php echo $datos['nombre_secretario']; ?>, adjuntando la documentación que considere necesaria para justificar su pedido.</p>
...
<?php if($datos['publicar_resultados'] == 'S') : ?>
	<?php if (!$datos['dictamen']['comision']) : ?>
		<p class="resultado_seccion">No se encontró un dictamen para esta postulación</p>
	<?php else : ?>
		<br><br>
		<fieldset>
			...
			<div style='font-size: 1.5em; padding-left: 20px; color:#39397d;'>
				...
				<?php foreach($datos['dictamen']['comision']['desglose_puntajes_con_descripciones'] as $criterio => $detalles_criterio) : ?>
					<div class="criterio">
						<h2><?php echo $criterio; ?> (Máximo: <?php echo $detalles_criterio['puntaje_maximo_criterio']; ?>)</h2>
					</div>
					<div>
						<table class="tabla_puntajes">
							<tr class="cabecera">
								<td>Criterio</td>
								<td>Máximo posible</td>
				...
				<?php if ($this->soy_admin() && count($datos['dictamen']['comision']['evaluadores'])) : ?>
					<p>
						<u><b>Participaron de la evaluación: (visible solo para administradores)</b></u>
						<ul>
				...
				<p>
					...
					<?php echo $datos['dictamen']['comision']['justificacion_puntajes']; ?>
				</p>
			</div>
		</fieldset>
				...
				<p>
					...
					<?php if(count($datos['dictamen']['junta'])) : ?>
						...
						<?php foreach($datos['dictamen']['junta']['desglose_puntajes'] as $puntaje) : ?>
							<li>
								<?php echo $puntaje['criterio_evaluacion']; ?>: <b><?php echo $puntaje['puntaje_asignado']; ?> puntos</b> (Máximo <?php echo $puntaje['puntaje_maximo']; ?>).
							</li>	
						<?php endforeach; ?>
						</ul>
					...
					<?php else: ?>
						<p><b>La Junta Coordinadora ha ratificado los puntajes asignados por la Comisión Asesora</b></p>
					<?php endif; ?>	
				</p>
			</div>
		</fieldset>
	...
	<?php if (($datos['admisibilidad']['beca_otorgada'] == 'S') && (file_exists($ruta_base . $ruta_pdf))): ?>
		<div style='text-align:center;'>
			<a target="_blank" href='<?php echo $url_base . $ruta_pdf; ?>' style="font-size: 2rem; text-decoration:none; background-color: #3c6eab; color:#FFF; padding: 5px 30px;">Ver resolución de otorgamiento</a>
		</div>
	<?php endif; ?>
	<br><br>
	<?php if(count($datos['cumplimientos'])): ?>
		<fieldset>
			...
			<div style='font-size: 1.5em; padding-left: 20px; color:#39397d;'>
				<p>A continuación se detalla la carga de cumplimientos de obligaciones por parte de su director/co-director.</p>
			</div>
			<ul>
