...
class form_inscripcion extends sap_ei_formulario
{
	...
	function extender_objeto_js()
	{
		...
		// Obtengo el promedio histórico de los graduados de los últimos años
		...
		function setear_prom_hist(respuesta)
		{
			if( ! respuesta.error){
				...
				//Si hubo error es porque no se encontró el promedio de la carrera seleccionada
				{$this->objeto_js}.ef('prom_hist_egresados').resetear_estado();
				{$this->objeto_js}.ef('prom_hist_egresados').set_solo_lectura(false);
			}
		}
		...
		{$this->objeto_js}.evt__validar_datos = function()
		{
			...
			//Si se asigna un codirector, debe justificarse su inclusión (se hace obligatorio el campo)
			...
			//Si se asigna un sub-director, debe justificarse su inclusión (se hace obligatorio el campo)
