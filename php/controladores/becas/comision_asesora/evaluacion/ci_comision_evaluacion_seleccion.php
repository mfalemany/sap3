...
class ci_comision_evaluacion_seleccion extends sap_ci
{
	...
	function evt__guardar()
	{
		...
		// Recorre todos los subpuntajes guardados y obtiene el número final (y lo asigna a be_dictamen)
		...
		try {
			...
			$this->log('Código error: ' . $time . ' | ' . $e->get_mensaje_motor(),'ComisionAsesora');
			toba::notificacion()->agregar('Ocurrió un error al intentar guardar. Por favor, comuniquese con personal de la SGCyT - UNNE para que lo ayude a resolverlo (Código de error: ' . $time . ')','error');
		} catch (Exception $e) {
			$this->log('Código error: ' . $time . ' | ' . $e->getMessage(),'ComisionAsesora');
			toba::notificacion()->agregar('Ocurrió un error al intentar guardar. Por favor, comuniquese con personal de la SGCyT - UNNE para que lo ayude a resolverlo (Código de error: ' . $time . ')','error');
		}
	}
	...
	function conf__pant_edicion(toba_ei_pantalla $pantalla)
	{
		...
		$subtipo_beca = json_decode($detalles['postulante']['informacion_interna'], true);
		
		$subtipo_beca_desc = '';
		if (isset($subtipo_beca['subtipo_beca'])) {
			$subtipo_beca_desc = co_becas::SUBTIPOS_BECA[$subtipo_beca['subtipo_beca']];
		}

		//si el tipo de beca contempla el puntaje acad?ico, lo muestro al evaluador
		$puntaje = ($detalles['beca']['suma_puntaje_academico'] == 'S') ? $detalles['postulante']['puntaje'] : "0";
		...
		$datos = array(
			...
			'subtipo_beca_desc'  => $subtipo_beca_desc,
	...
	function conf__form_dictamen(sap_ei_formulario $form)
	{
		...
		if (!isset($datos['solicitud_categoria_codir']) || !$datos['solicitud_categoria_codir']) {
		...
		if ( ! ( 
			(isset($datos['solicitud_categoria_dir'])    && $datos['solicitud_categoria_dir']) || 
			(isset($datos['solicitud_categoria_codir'])  && $datos['solicitud_categoria_codir']) ||
			(isset($datos['solicitud_categoria_subdir'])  && $datos['solicitud_categoria_subdir'])
		)
	...
	public function ajax__registrar_puntajes($datos, toba_ajax_respuesta $respuesta)
	{
		try {
			...
			$respuesta->set(['status' => false, 'mensaje' => 'No hay un dictamen activo en evaluación. Por favor, comuniquese con la SGCyT antes de seguir evaluando']);			
		}
	}
	...
	function extender_objeto_js()
	{
		...
		function determinar_visibilidad_ef_motivo_rechazo(){
			...
			// SI alguna de las solicitudes se rechazó, mostrar el campo motivo rechazo
		...
		{$this->objeto_js}.enviarPuntajes_listenResponse = function(respuesta){
			if (!respuesta.status) {
				...
				notificacion.mostrar_ventana_modal('Ocurrió un error', respuesta.mensaje, '500px');
			}
		}
		...
		{$this->objeto_js}.evt__guardar = function(event){
			...
			if (totalGeneral === 0) {
				if (!confirm('Si guarda una evaluación sin puntajes, estos se considerarán como ceros')) {
					{$this->objeto_js}.enviarPuntajes();
