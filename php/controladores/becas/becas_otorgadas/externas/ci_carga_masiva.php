...
class ci_carga_masiva extends sap_ci
{
	...
	protected function procesar_archivo($archivo,$detalles_convocatoria){
		...
		while($linea = fgets($archivo)){
			...
			//Esta linea elimina lineas vacías (se generan por los saltos de linea en el titulo de plan de beca)
			...
			if( ! $this->cabeceras){
				...
				foreach ($cabeceras_obligatorias as $obligatoria) {
					if( ! array_key_exists($obligatoria, $this->cabeceras )){
						//Si alguna de las cabeceras obligatorias no existe, no se puede seguir. Como dice "La Chiqui": Así, no!
			...
			//Si está establecida la cabecera area_conocimiento, se usa
			...
			//Si está establecida la cabecera dependencia, se usa
			...
			//Si está establecida la cabecera titulo_plan_beca, se usa
		...
		if(count($this->errores)){
			...
			toba::notificacion()->agregar('Hasta que estos problemas sean resueltos, no se importará ninguna beca. Este proceso inserta todas las becas, o ninguna.', 'warning');
			return FALSE;
		}else{
			return $becas;
		}
