...
class ci_comunicacion_edicion extends sap_ci
{
	...
	function conf__form_com_cabecera(sap_ei_formulario $form)
	{
		...
		if (!$this->soy_admin()) {
			$form->desactivar_efs(array('asistio_jornada'));
		}
	...
	function conf__cu_evaluaciones(sap_ei_cuadro $cuadro)
	{
		...
		if ($comunicacion) {
			$becarios_ven_evaluaciones = toba::consulta_php('co_comunicaciones')->becarios_pueden_ver_evaluaciones($comunicacion['sap_convocatoria_id']);
			if (!$this->soy_admin()) {
				if (!$becarios_ven_evaluaciones) {
	...
	function evt__cu_evaluaciones__eliminar($datos)
	{
		$this->dep('comunicacion_evaluacion')->cargar($datos);
		$this->dep('comunicacion_evaluacion')->eliminar();
		$this->dep('comunicacion_evaluacion')->resetear();
	}

	function conf_evt__cu_evaluaciones__eliminar(toba_evento_usuario $evento,$fila)
	{
		if ($this->soy_admin()) {
			$evento->mostrar();
		} else {
	...
	{
		if ($this->soy_admin()) {
			$this->agregar_notificacion('Los formularios y botones están disponibles solo porque el usuario es administrador');
			return;
		}
