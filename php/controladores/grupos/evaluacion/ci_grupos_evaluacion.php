...
class ci_grupos_evaluacion extends sap_ci
{
	...
	function evt__cu_grupos__seleccion($seleccion)
	{
		...
		// Agrego al filtro, el DNI del usuario logueado (cada evaluador hace su propia evaluación)
	...
	function evt__guardar()
	{
		try {
			...
			$this->agregar_notificacion('Ocurrió un error: ' . $e->get_mensaje() . $e->get_mensaje_motor());
		} catch (Exception $e) {
			$this->agregar_notificacion('Ocurrió un error: ' . $e->getMessage());
		}
	...
	public function evt__cerrar_evaluacion()
	{
		...
		if (empty($evaluacion)) {
			throw new toba_error('Debe cargar un resultado para su evaluación del último plan de trabajo, en la pesataña \'Evaluación\'');
		}
		

		$grupo = $this->get_datos('grupo')->get();
		if (empty($grupo['id_categoria'])) {
			throw new toba_error('Debe asignar una categoría al grupo (Emergente o Consolidado), en la pesataña \'Evaluación\'');
		}
		...
		$this->agregar_notificacion('Se ha cerrado su evaluación con éxito!');
		$this->evt__guardar();
	...
	public function servicio__ver_ultimo_informe()
	{
		//Cargo la clase encargada de generar el reporte
		$punto  = toba::puntos_montaje()->get('proyecto');
		...
		if (! isset($params['id'])) {
			echo "El proyecto seleccionado no tiene informes registrados";
			return;
		}
		...
		if (! array_key_exists($params['tipo'], $tipos_proyecto)) {
			echo "El tipo de proyecto seleccionado no es válido (o no registra informes en SAP)";
			return;	
		}

		$tipo = $tipos_proyecto[$params['tipo']];

		try {
			//Obtengo el ID del último informe del proyecto
			$id_informe = toba::consulta_php('co_proyectos_informes')->get_id_ultimo_informe_proyecto($tipo, $params['id']);
			...
			$informe = toba::consulta_php('co_proyectos_informes')->get_reporte_informe($tipo, $id_informe);

			$reporte = new Reporte_informe_proyecto($informe);
			
			$reporte->mostrar();
		} catch (toba_error_db $e) {
			$codigo_error = substr(time(), -4);
			echo "Ocurrió un error al intentar obtener el último informe. Codigo de error: " . $codigo_error;
			$this->log($e->get_mensaje_motor() . ' || ' . $e->get_mensaje(), 'grupos_eval');
		}
