...
/*		$rol_camposDB = [
	...
	private function loguear_escenarios_no_deseados($categorias, $nro_documento){
		...
		// Si count($categorias) > 1 es porque a una persona se la evaluó con/se le concedieron dos categorias distintas
		...
		if (count($categorias) > 1) {
			...
			toba::consulta_php('helper_archivos')->log('El director ' . $director . ' fue evaluado con dos categorías distintas: ' . $categorias_concedidas, 'EVAL_CAT_TRANSITORIA');
		}
	...
	/**
	 * Este función es útil para obtener solamente las dependencias declaradas en una convocatoria de becas.
	 * Sirve para usar en combos de selección, y así evitar listar al usuario un monton de dependencias
	...
	public function get_detalle_informes_por_convocatoria_y_tipo_beca($id_convocatoria, $id_tipo_beca)
	{
		...
		for ($i=1; $i <= $cantidad_informes_a_presentar ; $i++) { 
			...
			// Calculo la fecha a partir de la cual deberìa poder presentarse el informe
