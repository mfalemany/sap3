...
class CVFromCvar extends FPDF
{
		...
		if (isset($this->cargos['cargosDocencia'])) {
			...
			if (isset($cargos['superiorUniversitarioGradoPosgrado']['superiorUniversitarioGradoPosgrado'])) {
				...
				foreach ($cargosUniversitarios as $cargo) {
					...
					$this->Cell($this->getAnchoData()-4,  $this->alto_linea, strtolower($cargo['dedicacionHorariaSemanal']), $this->borde, 1, 'L', false);
		...
		if (isset($this->cargos['cargosOtros'])) {
			...
			if (isset($cargos['cargoOtro'])) {
				...
				foreach ($cargos as $cargo) {
					...
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Cargo:', $this->borde, 0, 'L', false);
					$this->MultiCell($this->getAnchoData()-4,  $this->alto_linea, $cargo['cargo'], $this->borde, 'L', false);
					$this->setFont($this->fuente_familia, '', $this->fuente_tamanio);
	...
	protected function datos_antecedentes()
	{
		$this->titulo_seccion('Antecedentes');
		

		if (isset($this->antecedentes['financiamientoCT'])) {
			$antecedentes = $this->antecedentes['financiamientoCT'];

			if (isset($antecedentes['proyectosID'])) {
				$this->subtitulo_seccion('Financiamiento de Proyectos');
				$this->ancho_etiqueta = 27;
				// Esta validación se debe a que CVAr no devuelve los datos de manera uniforme.
				// Si existe un solo cargo, la devuelve directamente (como unico elemento), pero
				// si existe mas de una, llega un array (entonces esto hace homogeneo el resultado: lo pasa a array si es un solo elemento)
				$indices = array_keys($antecedentes['proyectosID']['proyectoID']);
				if (is_numeric($indices[0])) {
					$proyectos = $antecedentes['proyectosID']['proyectoID'];
				} else {
					$proyectos = [$antecedentes['proyectosID']['proyectoID']];
				}
				...
				foreach ($proyectos as $proyecto) {
					...
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Título:', $this->borde, 0, 'L', false);
					$this->MultiCell($this->getAnchoData()-4,  $this->alto_linea, $proyecto['titulo'], $this->borde, 'L', false);
					$this->setFont($this->fuente_familia, '', $this->fuente_tamanio);

					//Descripción
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Descripción:', $this->borde, 0, 'L', false);
					$this->MultiCell($this->getAnchoData()-4,  $this->alto_linea, $proyecto['descripcion'], $this->borde, 'J', false);

					//Fecha inicio
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Fecha Desde:', $this->borde, 0, 'L', false);
					$this->Cell($this->getAnchoData()-4,  $this->alto_linea, $this->getFecha($proyecto['fechaDesde'],'d-m-Y'), $this->borde, 1, 'L', false);

					//Fecha Fin
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Fecha Hasta:', $this->borde, 0, 'L', false);
					$this->Cell($this->getAnchoData()-4,  $this->alto_linea, $this->getFecha($proyecto['fechaHasta'],'d-m-Y'), $this->borde, 1, 'L', false);

					//Monto
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Monto:', $this->borde, 0, 'L', false);
					$this->Cell($this->getAnchoData()-4,  $this->alto_linea, $proyecto['moneda']['simbolo'] . number_format($proyecto['monto'], 2), $this->borde, 1, 'L', false);

					//Instituciones Participantes
					// Esta validación se debe a que CVAr no devuelve los datos de manera uniforme.
					// Si existe un solo cargo, la devuelve directamente (como unico elemento), pero
					// si existe mas de una, llega un array (entonces esto hace homogeneo el resultado: lo pasa a array si es un solo elemento)
					$indices = array_keys($proyecto['institucionesParticipantes']['institucionesParticipantes']);
					if (is_numeric($indices[0])) {
						$proyectos = $proyecto['institucionesParticipantes']['institucionesParticipantes'];
					} else {
						$proyectos = [$proyecto['institucionesParticipantes']['institucionesParticipantes']];
					}
					$instituciones = implode(', ', array_column(array_column($proyectos, 'organizacion'), 'rutaCompleta'));
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Instituciones:', $this->borde, 0, 'L', false);
					$this->MultiCell($this->getAnchoData()-4,  $this->alto_linea, $instituciones, $this->borde, 'L', false);

					//Palabras clave
					$this->setX($this->getX()+4);
					$this->Cell($this->ancho_etiqueta, $this->alto_linea, 'Palabras clave:', $this->borde, 0, 'L', false);
					$this->MultiCell($this->getAnchoData()-4,  $this->alto_linea, implode(', ', array_column($proyecto['palabrasClave']['palabrasClave'], 'palabraClave')), $this->borde, 'L', false);
