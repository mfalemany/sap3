...
class Reporte_informe_proyecto extends FPDF
{
	...
	function detalles_proyecto($proyecto)
	{
		...
		$periodo = sprintf('%s años (%s al %s)',$duracion, $desde, $hasta);
		...
		//Periodo
		$this->imprimir_etiqueta('Duración: ');
	...
	function detalles_informe($informe)
	{
		...
		$this->imprimir_etiqueta('Fecha de presentación: ');
	...
	{
		$this->imprimir_titulo_cuadro('Evaluación de integrantes');
		$this->SetFont('arial','',10);
	...
	{
		$this->imprimir_titulo_cuadro('Producción (' . count($trabajos) . ' elementos)');
		$this->SetFont('arial','',10);
		...
		foreach($trabajos as $trabajo){
			...
			if($trabajo['info_complementaria']){
				$this->imprimir_etiqueta('Información adicional: ',0);
	...
	{
		$this->imprimir_titulo_cuadro('Difusión (' . count($trabajos) . ' elementos)');
		$this->SetFont('arial','',10);
	...
	function trabajos_transferidos($trabajos)
	{
		...
		foreach($trabajos as $trabajo){
			//Tipo de informe
			$this->imprimir_etiqueta('Descripción: ',0);
	...
	function becarios($internos, $externos){
		$this->imprimir_titulo_cuadro('Formación de Recursos Humanos (' . (count($internos)+count($externos)) . ' elementos)');
		...
		foreach($internos as $becario){
			...
			//Plan de beca
			$this->imprimir_etiqueta('Título plan beca: ',0);
		...
		foreach($externos as $becario){
			...
			$this->Cell(190-$this->ancho_etiqueta,$this->alto_linea,$becario['becario'],0,1,'L',false);
			//Director
			$this->imprimir_etiqueta('Dirección: ',0);
			$this->Cell(190-$this->ancho_etiqueta,$this->alto_linea,$becario['director'],0,1,'L',false);
			//Plan de beca
			$this->imprimir_etiqueta('Título plan beca: ',0);
			$this->MultiCell(190-$this->ancho_etiqueta,$this->alto_linea,$becario['titulo_plan_beca'],0,'L',false);
	...
	{
		$this->imprimir_titulo_cuadro('Gestión del proyecto');
		$this->SetFont('arial','',10);
		$this->Ln();
		if( ! ($informe['equipam_mejoro_capac'] || $informe['estim_efic_inversiones'] || $informe['dificult_gestion']) ){
			$this->imprimir_titulo_seccion('No se han declarado detalles sobre la gestión del proyecto',0);
		}
		if($informe['equipam_mejoro_capac']){
			$this->imprimir_titulo_seccion('Medida en que el equipamiento adquirido mejoró la capacidad del grupo y de la institución',0);
			$this->MultiCell(190,$this->alto_linea,$informe['equipam_mejoro_capac'],0,'J',false);
			$this->Ln();
		}
		if($informe['estim_efic_inversiones']){
			$this->imprimir_titulo_seccion('Ajuste entre lo presupuestado y lo efectivamente gastado o invertido, en función de las metas alcanzadas',0);
			$this->MultiCell(190,$this->alto_linea,$informe['estim_efic_inversiones'],0,'J',false);
			$this->Ln();
		}
		if($informe['dificult_gestion']){
			$this->imprimir_titulo_seccion('Dificultades encontradas en la gestión del proyecto',0);
	...
	function metas_alcanzadas($metas)
	{
		...
		foreach($cumplidas as $objetivo => $metas){
			$this->imprimir_titulo_seccion('Objetivo específico: ' . $objetivo);
			foreach($metas as $indice => $meta){
				...
				//No es la última meta, entonces se hace un salto de linea para que no queden tan juntos
