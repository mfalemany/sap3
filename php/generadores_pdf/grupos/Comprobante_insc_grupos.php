...
class Comprobante_insc_grupos extends FPDF
{
	function __construct($datos = array())
	{
		...
		$this->saltar(2);
		$this->SetFont('Arial','BU',10);
		$this->Cell(45,$alto_linea,"Integrantes:",0,0,'R');
		$this->SetFont('Arial','',10);
		$this->Cell(0,$alto_linea,"(firmar)",0,0,'L');
		$alto_linea += 3;
		$this->Cell(0,$alto_linea," ",0,1,'L');

		foreach ($datos['integrantes'] as $integrante) {
			$this->Cell(45,$alto_linea,"....................................",0,0,'C');
		...
		$this->Rect($this->getX(),$y_inicio,190, ($this->getY()-$y_inicio+5) );

		//FIRMAS
		$this->setY($this->getY()+40);
		$this->Line(65,$this->getY(),145,$this->getY()); //Linea gris
		$this->setY($this->getY()+2);
		$this->Cell(0,7,'Aval Sec. Investigación / Dir. del Instituto',0,0,'C');
