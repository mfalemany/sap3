...
class helper_archivos
{
	...
	/**
	 * Obtiene y retorna, a partir del tipo MIME del archivo, su extensión
	 * @param  string  Ruta hacia el archivo del cuál se quiere obtener la extensión
	 * @return mixed
	 */
	...
	function subir_archivo($detalles,$carpeta,$nombre_archivo,$tipos_permitidos = array())
	{
		...
		if(count($tipos_permitidos)){
			...
			//Si no se especificó ningun tipo de archivo en particular
			return move_uploaded_file($detalles['tmp_name'], $this->ruta_base().$carpeta."/".$nombre_archivo);
		}
	}
	...
	//Función que registra una linea de log
	...
	/* =============== EXISTE PDF? (Documentación Personl) ===================== */
	...
	public function get_path_documento_personal($nro_documento, $cual)
	{
		...
		// Retorno la ruta
		return $this->url_base() . $ruta_documento . '?v=' . time(); //TODO: evitar el caché desde apache
	}
