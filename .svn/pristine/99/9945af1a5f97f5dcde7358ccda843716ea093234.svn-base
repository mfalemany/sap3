...
class co_personas{
	...
	function get_personas_ef_editable($patron){
		...
		$patron = toba::consulta_php('helper')->quitar_acentos($patron);
		$sql = "SELECT per.nro_documento, 
				...
				WHERE quitar_acentos(per.apellido) ILIKE ".quote('%'.$patron.'%')."
				OR quitar_acentos(per.nombres) ILIKE ".quote('%'.$patron.'%')." 
				OR nro_documento ilike ".quote('%'.$patron.'%')." 
				OR quitar_acentos(per.apellido)||' '||quitar_acentos(per.nombres) ilike ".quote('%'.$patron.'%')." 
				OR quitar_acentos(per.nombres)||' '||quitar_acentos(per.apellido) ilike ".quote('%'.$patron.'%')." 
	...
	function get_personas($filtro = array())
	{
		...
		if (!empty($filtro['cuil'])) {
			$cuil   = preg_replace("/[\r\n|\n|\r]+/", ",", $filtro['cuil']);
			$cuil   = str_replace(['-',' ','.'], '', $cuil);
			$cuiles = explode(',', $cuil);
			$cuiles = array_map(function($elem){ return quote($elem); }, $cuiles);
			$cuiles = implode(',',$cuiles);
			$where[] = "per.cuil IN ($cuiles)";
		}
	...
	public function tiene_cargo_conicet_con_asiento_en_unne($nro_documento)
	{
		$nro_documento = str_replace('.','',$nro_documento);
		$sql = "SELECT lugar_trabajo_tiene_asiento_en_unne 
				FROM be_cat_conicet_persona 
				WHERE nro_documento = ".quote($nro_documento);
		$resultado = toba::db()->consultar_fila($sql);
		return (count($resultado) && $resultado['lugar_trabajo_tiene_asiento_en_unne'] == true);
	}
