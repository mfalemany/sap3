...
class co_grupos
{
	...
	/**
	 * Determina si una persona fue asignada como evaluador de algun grupo de investigación.
	...
	{
		$sql = "SELECT gt.*, tt.tipo_transferencia, CASE gt.sector WHEN 'R' THEN 'Privado' ELSE 'Público' END AS sector_desc
	...
	/**
	 * Obtiene y retorna un listado de becarios de la SGCyT que están incluídos como integrantes en proyectos de
	 * investigación declarados por un grupo de investigación (formación de recursos humanos del grupo)
	 * @param  int $id_grupo
	 * @return array
	 */
	...
	/**
	 * Los estados enumerados por este método, son los estados que NO SE CONSIDERAN al momento de determinar
	...
	function get_evaluaciones($id_grupo)
	{
		...
		$sql = "SELECT *, CASE gie.resultado WHEN 'A' THEN 'Aprobado' WHEN 'D' THEN 'Desaprobado' WHEN 'P' THEN 'En proceso de evaluación' ELSE 'Sin Evaluación' END as resultado_desc
	...
	function get_grupos_a_evaluar($filtro = array())
	{
		...
		$sql = "SELECT $conv AS id_convocatoria,
				...
				WHERE gr.id_estado IN ($estados_eval) -- El grupo debe estar en estado de evaluación
	...
	function get_grupos($filtro = array())
	{
		...
		//Los grupos acreditados son aquellos que tuvieron la ?ltima evaluación positiva (y al menos una). No se incluyen entonces a: grupos que no tienen ninguna evaluación, o que teniendo evaluaciones, la ?ltima no es aprobada
		...
		$sql = "SELECT gr.*,
						...
						CASE (" . $estado_evaluacion_ultimo_plan . ") 
							...
							WHEN 'P' THEN 'En proceso de evaluación'
							WHEN 'D' THEN 'Desaprobado'
							ELSE 'Sin evaluación' END AS estado_evaluacion_ultimo_plan,
	...
	public function permite_cambio_categoria($id_grupo)
	{
		if (!is_numeric($id_grupo)) {
			throw new toba_error('No se encontró un grupo con el identificador proporcionado');
		}
