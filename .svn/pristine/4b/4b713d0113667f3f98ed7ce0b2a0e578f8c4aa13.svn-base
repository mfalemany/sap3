...
class Plan_trabajo_grupo extends FPDF
{
	...
	function __construct($datos = array())
	{
		...
		$items = [
			['etiqueta' => 'Extensión'           , 'indice' => 'extension'    ],
			['etiqueta' => 'Publicaciones'       , 'indice' => 'publicaciones'],
			['etiqueta' => 'Transferencia'       , 'indice' => 'transferencia'],
			['etiqueta' => 'Formación RR.HH.'    , 'indice' => 'form_rrhh'    ],
			['etiqueta' => 'Organización Eventos', 'indice' => 'org_eventos'  ],
		...
		foreach ($items as $item) {
			...
			if ($datos[$item['indice']]) {
				...
				$this->MultiCell(0,$alto_linea,'Sin información declarada',0,'J');
			}
	...
	function Header()
	{
		...
		//Se agrega la convocatoria (es el aÃ±o de la fecha de inscripciÃ³n) y la fecha de impresiÃ³n (ambas en un lugar fijo)
		$this->SetTextColor(150,150,150);
		$fecha = date('d-m-Y', strtotime($this->datos['fecha_presentacion']));
		$this->Text(140,29,'Fecha de presentación: ' . $fecha);
	}

	function Footer()
	{
		$this->setY(280);
		$this->Cell(0,5,'Secretaría General de Ciencia y Técnica - UNNE', 0,1,'C');
