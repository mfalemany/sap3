...
class ci_detalles_proyecto extends sap_ci
{
		...
		try {
			...
			if($es_definitivo){
				...
				if ($resultado_validacion['status'] == false) {
					toba::notificacion()->error('El proyecto no está en condiciones de ser presentado de forma definitiva. Por favor, verifique los detalles en la pestaña \'Control de Condiciones\'');
	...
	function validar_proyecto()
	{
		...
		if(count($director)){
			...
			if($resultado !== TRUE){
				$errores['direccion'][] = 'Validación del Director: ' . $resultado;
			}else{
				$integrantes_validos++;
			}
		}
		...
		if(count($codirector)){
			...
			if($resultado !== TRUE){
				$errores['direccion'][] = 'Validación del Co-Director: ' . $resultado;
			}else{
				$integrantes_validos++;
			}
		}
		...
		if(count($subdirector)){
			...
			if($resultado !== TRUE){
				$errores['direccion'][] = 'Validación del Sub-Director: ' . $resultado;
			}else{
				$integrantes_validos++;
			}
		}
		...
		if(count($integrantes) - $integrantes_validos - $inv_externos < 1){
			$errores['integrantes'][] = 'El proyecto debe tener al menos un integrante que no sea Director, Co-Director, Sub-Director ni Docente Investigador (Tesista, Alumno, Becario, Personal Técnico-Profesional). No se consideran los Investigadores Externos';
		}
		...
		foreach($integrantes as $integrante){
			...
			if(toba::consulta_php('co_proyectos')->get_identificador_perfil($integrante['id_funcion']) == 'I'){
				if( ! toba::consulta_php('co_personas')->es_docente($integrante['nro_documento'])){
					...
					$errores[] = "El integrante $ayn está definido como Docente Investigador, y no cuenta con cargos docentes activos en la UNNE";
				}
			}
		}
		return $errores;
	...
	function validar_pertenencia_proyectos($integrantes)
	{
		...
		foreach ($integrantes as $integrante) {
			if( ! toba::consulta_php('co_proyectos')->puede_integrar_nuevo_proyecto($integrante['nro_documento'],array($id_proyecto))){
				...
				$errores[]        = "El integrante $ayn ya forma parte de ".count($proyectos)." proyectos. Solo puede integrar, como máximo, dos proyectos. Los proyectos que integra son: $lista";
			}
		}
		return $errores;
	}
		...
		foreach($reglas as $regla){
			...
			if($regla->min_registros){
				if(count($registros) < $regla->min_registros){
					$errores[] = "Se debe cargar, como mínimo {$regla->min_registros} registros en \"{$regla->nombre_mostrar}\". Se encontraron " . count($registros);
				}
			}
		...
		foreach ($perfiles_validacion as $identificador_perfil => $condiciones) {
			...
			if($condiciones['obligatorio']){
				...
				if( ! array_key_exists($funcion['id_funcion'], $perfiles)){
					$errores[] = "Debe existir un integrante que tenga asigada la función de {$funcion['funcion']}";
				}
			}
			//Debería ser unico?
			if($condiciones['unico']){
				...
				if( isset($perfiles[$funcion['id_funcion']]) && $perfiles[$funcion['id_funcion']] > 1){
					$errores[] = "No puede existir mas de un integrante con la función de {$funcion['funcion']}";
				}
			}
		}
		return $errores;
	...
	function validar_registros_auxiliares($integrantes)
	{
		...
		foreach($integrantes as $integrante){
			if( ! toba::consulta_php('co_proyectos')->tiene_registro_auxiliar($integrante) ){
				$ayn       = $this->get_ayn($integrante['nro_documento']);
				$errores[] = "El integrante $ayn no tiene información necesaria en la solapa 'Recursos Humanos'";
			}
		}
		return $errores;
	}
