...
class ci_evaluacion_informes extends sap_ci
{
	...
	function conf__form_evaluacion(sap_ei_formulario $form)
	{
		...
		if (!empty($datos['fecha'])) {
			...
			// TODO: descomentar esto despues del 10/03/2023 (tiempo para evaluar informe de Niklas)
			/*if ($tiempo_transcurrido->days > 90) {
				$form->set_solo_lectura();
				$form->agregar_notificacion('No se pueden modificar evaluaciones que hayan sido realizadas hace mas de 90 días.');
		...
		// TODO: Sacar esto despues que la Junta Coordinadora evalúe informes
		$form->desactivar_efs('evaluadores');
	}
	...
	function evt__form_evaluacion__modificacion($datos)
	{
		...
		// TODO: Sacar esto despues que la Junta Coordinadora evalúe informes
		$datos['evaluadores'] = toba::usuario()->get_id();
