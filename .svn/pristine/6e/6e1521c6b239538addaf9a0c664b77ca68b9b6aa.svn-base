...
class ci_login extends toba_ci
{
	...
	{
		// Si existe notificaci? pendiente, se muestra en pantallaensa
	...
	function conf__form_passwd_vencido($form)
	{
		...
		$form->ef('clave_nueva')->set_descripcion("La clave debe tener al menos $largo_clave caracteres, entre letras mayúsculas, minúsculas, números y síbolos, no pudiendo repetir caracteres adyacentes");
	...
	{
		//Si tiene mas de 15 caracteres, no es un dni, y no tiene sentido esta busqueda
		if(strlen($nro_documento) > 15){
			return;
		}
		
		//Obtengo los perfiles actualmente asignados al usuario (no se usa toba::usuario()->get_perfiles_funcionales() porque en este punto retorna siembre vacío)
		$sql = "SELECT usuario_grupo_acc FROM apex_usuario_proyecto WHERE proyecto = 'sap' AND usuario = " . quote($nro_documento);
		$perfiles_usuario = toba::instancia()->get_db()->consultar($sql);
		if (count($perfiles_usuario)) {
			$perfiles_usuario = array_column($perfiles_usuario, 'usuario_grupo_acc');
		}
		
		//Si es un administrador, no necesita automatizarse la asignacion de perfiles
		if (in_array('admin', $perfiles_usuario)) {
			return;
		}

		//Defino todos los perfiles que deben ser evaluados para asignar
		$perfiles_condiciones = array(
			array('perfil' => 'integrante_comision_asesora', 'condiciones' => array(
				array('clase' => 'co_comision_asesora','metodo' => 'es_integrante_comision_asesora','condicion' => "===TRUE")
			)),
			array('perfil' => 'becas',                'condiciones' => array()),
			array('perfil' => 'solicitante_subsidios','condiciones' => array(
			 	array('clase' => 'co_personas','metodo' => 'es_docente',              'condicion' => "===TRUE"),
				array('clase' => 'co_personas','metodo' => 'tiene_mayor_dedicacion',  'condicion' => "===TRUE"),
				array('clase' => 'co_personas','metodo' => 'get_categoria_incentivos','condicion' => " == TRUE")	
			)),
			array('perfil' => 'director_proyectos',   'condiciones' => array(
			 	array('clase' => 'co_personas','metodo' => 'es_docente',              'condicion' => "===TRUE")
				
			)),
			/*VA EN UNA LINEA SEPARADA DE LA ANTERIOR PORQUE SE DEBEN CUMPLIR TODAS LAS CONDICIONES JUNTAS*/
			array('perfil' => 'director_proyectos', 'condiciones' => array(
				array('clase' => 'co_proyectos','metodo' => 'esta_exceptuado',        'condicion' => "==TRUE")
			)),
			array('perfil' => 'coordinador_grupos', 'condiciones' => array(
				// array('clase' => 'co_grupos','metodo' => 'puede_coordinar',        'condicion' => "===TRUE"),
				array('clase' => 'co_grupos','metodo' => 'puede_ver_opcion_grupos','condicion' => "===TRUE")	
			)),
			array('perfil' => 'director_beca',      'condiciones' => array(
				array('clase' => 'co_becas','metodo' => 'dirige_beca_otorgada','condicion' => "==TRUE")
			)),
			array('perfil' => 'avalista_becas',      'condiciones' => array(
				array('clase' => 'co_becas','metodo' => 'es_director_beca','condicion' => "==TRUE")
			)),
			array('perfil' => 'avalista_becas',      'condiciones' => array(
				array('clase' => 'co_proyectos','metodo' => 'dirige_proyectos','condicion' => "===TRUE")
			)),
			array('perfil' => 'avalista_becas',      'condiciones' => array(
				array('clase' => 'co_personas', 'metodo' => 'es_autoridad',   'condicion' => "===TRUE")
			)),
			array('perfil' => 'director_comunicacion','condiciones' => array(
				array('clase' => 'co_comunicaciones','metodo' => 'es_director_comunicacion','condicion' => "===TRUE")
			)),
			array('perfil' => 'evaluador_grupos','condiciones' => array(
				array('clase' => 'co_grupos','metodo' => 'es_evaluador_grupos','condicion' => "===TRUE")
			)),
		);
		
		foreach ($perfiles_condiciones as $perfil){

			//si el usuario no lo tiene asignado todavia...
			if(!in_array($perfil['perfil'],$perfiles_usuario)){
				
				//realizo la evaluación para ver si corresponde asignarle
				$corresponde = TRUE;
				
				foreach($perfil['condiciones'] as $condicion){
				...
				if($corresponde){
					try {
						toba::instancia()->vincular_usuario('sap',$nro_documento,$perfil['perfil']);
