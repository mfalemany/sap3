...
class ci_admisibilidad extends sap_ci
{
	...
	function conf__pant_edicion(toba_ei_pantalla $pantalla)
	{
		...
		// Puede que esto sea temporal (categorización transitoria de incentivos)
		$datos['solic_trans_inc_dir'] = toba::consulta_php('co_becas')->get_solicitud_transitoria_incentivos($insc['nro_documento_dir'], 1); // EL 1 está hardcodeado porque se hizo a las apuradas
		...
		if (!empty($insc['nro_documento_codir'])) {
			$datos['solic_trans_inc_codir'] = toba::consulta_php('co_becas')->get_solicitud_transitoria_incentivos($insc['nro_documento_codir'], 1); // EL 1 está hardcodeado porque se hizo a las apuradas
		...
		if (!empty($insc['nro_documento_subdir'])) {
			$datos['solic_trans_inc_subdir'] = toba::consulta_php('co_becas')->get_solicitud_transitoria_incentivos($insc['nro_documento_subdir'], 1); // EL 1 está hardcodeado porque se hizo a las apuradas
