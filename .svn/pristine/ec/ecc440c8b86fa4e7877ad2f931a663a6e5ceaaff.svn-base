...
class co_becas_informes
{
	...
	function get_detalles_informe($id_informe)
	{
		...
		$resultado['evaluadores'] = $this->get_evaluadores_informe($id_informe);
	...
	public function get_evaluadores_informe($id_informe)
	{
		//Obtengo los evaluadores
		$sql = "SELECT apellido, nombres, nro_documento 
				FROM sap_personas 
		...
		$sql = "SELECT 
			...
			FROM (
				SELECT oto.fecha_hasta, (oto.fecha_desde::date + (( (
						...
						/* TODO: cambiar el -3 por -1 despues del 01/01/2023 (se hizo por una excepción) */
						SELECT (meses_present_avance - 3) as meses_present_avance
						FROM be_tipos_beca 	
