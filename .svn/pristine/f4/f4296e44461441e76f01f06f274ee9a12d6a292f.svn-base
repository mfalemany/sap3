...
class co_comision_asesora
{
	...
	/**
	 * Retorna los criterios y subcriterios de evaluaciòn para una convocatoria y tipo de beca
	 *
	...
	function get_orden_merito($filtro = array())
	{
		...
		if(isset($filtro['id_tipo_beca'])){
			...
			// P/D: Si estás leyendo esto en el futuro, ya sabrás como nos fue en el mundial 
			// (hoy perdimos contra Arabia Saudita, un bajón) 
	...
	public function describir_desglose_puntajes($id_convocatoria, $id_tipo_beca, $json_desglose_puntajes)
	{
		...
		// Convierto el json en array y empiezo a reemplazar uno por uno, el ID por su descripción
		...
		foreach ($desglose_puntajes as $puntaje) {
			...
			// Se valida la existencia de subcriterio porque en dictámenes de junta no existe (evalúan solo el criterio general)
			if (isset($ids[1])) {
