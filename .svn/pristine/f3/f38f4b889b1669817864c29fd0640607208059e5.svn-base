...
class co_proyectos_informes
{
	...
	public function get_resultado_ultimo_informe($id_proyecto)
	{
		$id_proyecto = quote($id_proyecto);
		$sql = "SELECT COUNT(*) AS cantidad
				FROM sap_proy_pi_informe_eval 
				WHERE id_informe IN (
				    SELECT id_informe FROM sap_proyectos_pi_informe WHERE id_proyecto = $id_proyecto
				)
				AND estado = 'C'
				AND id_informe = (SELECT MAX(id_informe) FROM sap_proyectos_pi_informe WHERE  id_proyecto = $id_proyecto)
				AND satisfactorio = 'S'";
		$satisfactorios = toba::db()->consultar_fila($sql);
		...
		$sql = "SELECT COUNT(*) AS cantidad
				...
				AND id_informe = (SELECT MAX(id_informe) FROM sap_proyectos_pi_informe WHERE  id_proyecto = $id_proyecto)
				AND satisfactorio = 'N'";
		$no_satisfactorios = toba::db()->consultar_fila($sql);

		if ($satisfactorios['cantidad'] > $no_satisfactorios['cantidad']) {
	...
	/**
	 * Obtiene y retorna el ID del último informe del proyecto recibido como parámetro
	 *
	 * @param  int $id_proyecto
	 *
	 * @return int
	 */
	public function get_id_ultimo_informe_proyecto($tipo, $id_proyecto)
	{
		$id_proyecto = quote($id_proyecto);

		if (! in_array($tipo, ['pi', 'pdts'])) {
			return null;
		}

		$sql         = "SELECT MAX(id_informe) AS id_informe FROM sap_proyectos_{$tipo}_informe WHERE id_proyecto = $id_proyecto";
	...
	//Retorna un listado completo de informes, tanto de PI, como de PDTS. La idea es que ofrezca los datos b?icos y sirva solo para selecci?. Luego, se debera usar otro m?odo para obtener mas detalles del informe seleccionado
	...
	function get_reporte_informe($tipo, $id_informe)
	{
		...
		$sql = "SELECT dif.titulo, url, dif.evento, dif.anio, dif.url, dif.info_complementaria,
	...
	function get_trabajos_difundidos($tipo, $filtro = array()){
		 ...
		 *	Se utiliza para (generalmente) para mostrar al usuario informaci? que carg? en informes anteriores al actual (al que est? cargando)
		 */
	...
	function marcar_abierto($tipo,$id_informe)
	{
		$sql = 'UPDATE sap_proyectos_'.$tipo.'_informe SET estado = \'A\' , presentado_por = NULL, fecha_presentacion = NULL WHERE id_informe = ' . quote($id_informe);
