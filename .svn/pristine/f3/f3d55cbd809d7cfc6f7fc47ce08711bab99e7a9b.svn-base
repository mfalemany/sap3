...
class ci_inscripcion extends sap_ci
{
	...
	function generar_comprobante($clave)
	{
		...
		if ($this->soy_admin()) {
			...
			// Dictamen de Junta
			$detalles['dictamen_junta'] = toba::consulta_php('co_comision_asesora')->get_detalles_dictamen($clave, 'J');

			if (!empty($detalles['dictamen_junta'][0]['desglose_puntajes'])) {
				$detalles['dictamen_junta'][0]['desglose_puntajes_array'] = toba::consulta_php('co_comision_asesora')->describir_desglose_puntajes($clave['id_convocatoria'], $clave['id_tipo_beca'], $detalles['dictamen_junta'][0]['desglose_puntajes']);
			}
		}
	...
	function servicio__ver_comprobante()
	{
		...
		if (!$inscripcion) {
			die('No se ha seleccionado una inscripción para generar el comprobante');
		}
		...
		try{
			...
			switch ($e->get_sqlstate()) {
				...
				default:
					...
					toba::notificacion()->agregar('Ocurrió un error inesperado al intentar guardar la inscripción. por favor, comuniquese con la Secretaría General de Ciencia y Ténica para solucionarlo ' . ($mail_rrhh ? "($mail_rrhh)" : ''));	
					break;
			}
		} catch(Exception $e) {
			//logueo el error
			...
			$this->log($mensaje_log, 'becas');
			toba::notificacion()->agregar('Ocurrió un error: ' . $e->getMessage());
		}
