<?php
class ci_trab_transferencia_pdts extends sap_ci
{
	...
	function evt__guardar()
	{
		try {
			...
			$this->get_datos('pdts_inf_trab_transf')->set(array('id_informe'=>$informe['id_informe']));
	...
	{
		$trabs = toba::consulta_php('co_proyectos_informes')->get_trabajos_transferidos('pdts',array('id_informe'=>$this->informe_actual['id_informe']));
	...
	function evt__cu_trab_transferido__borrar($seleccion)
	{
		...
		try {
			$this->get_datos()->cargar($seleccion);
			$this->get_datos('pdts_inf_trab_transf')->cargar(array('id_informe'=>$this->informe_actual['id_informe']));
	...
	{
		$cuadro->colapsar();
		$trabs = toba::consulta_php('co_proyectos_informes')->get_trabajos_transferidos('pdts',array('previos_a_id_informe'=>$this->informe_actual['id_informe']));
		$cuadro->set_datos($trabs);
	}
